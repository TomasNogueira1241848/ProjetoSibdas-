...
$localizacao = null;
$localizacaoId = null;
$idLocalizacaoEncrypted = $_POST['id_localizacao'] ?? $_GET['id_localizacao'] ?? null;

if ($idLocalizacaoEncrypted !== null) {
    $idDesencriptado = aes_decrypt($idLocalizacaoEncrypted);
    $localizacaoId = ($idDesencriptado !== false && is_numeric($idDesencriptado)) ? (int) $idDesencriptado : null;
} elseif (isset($_GET['id']) && is_numeric($_GET['id'])) {
    /* Compatibilidade com links antigos, caso existam. */
    $localizacaoId = (int) $_GET['id'];
    $idLocalizacaoEncrypted = aes_encrypt($localizacaoId);
}
...
if ($ligacao === null) {
    ...
    try {
        ...
        $stmtLocalizacao = $ligacao->prepare('SELECT * FROM localizacoes WHERE id = :id LIMIT 1');
        $stmtLocalizacao->execute([':id' => $localizacaoId]);
        $localizacao = $stmtLocalizacao->fetch();
        ...
        $valores = [
            ...
            'tipoLocalizacao' => $localizacao->tipo_localizacao_id ?? '',
            'edificioLocalizacao' => $localizacao->edificio ?? '',
            'pisoPrincipalLocalizacao' => $localizacao->piso_principal ?? '',
            'numeroAndaresLocalizacao' => $localizacao->numero_andares ?? '',
            'estadoLocalizacao' => $localizacao->estado_localizacao_id ?? '',
            'responsavelLocalizacao' => $localizacao->responsavel ?? '',
            'telefoneLocalizacao' => $localizacao->telefone ?? '',
            'descricaoLocalizacao' => $localizacao->descricao ?? ''
        ];
    } catch (PDOException $erro) {
        $erroSistema = 'Ocorreu um erro ao carregar os dados da localização.';
    }
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && $ligacao !== null && $localizacao !== null) {
    foreach ($valores as $campo => $valorDefeito) {
        $valores[$campo] = trim((string) ($_POST[$campo] ?? $valorDefeito ?? ''));
    }

    /* Normalização dos dados principais, tal como no localizacao-nova.php */
    ...
    /* Validações em PHP: iguais às do localizacao-nova.php */
        ...
        try {
            $sql = "
                UPDATE localizacoes SET
                    codigo = :codigo,
                    nome = :nome,
                    tipo_localizacao_id = :tipo_localizacao_id,
        ...
        <main class="col-12 col-md-9 col-lg-10 p-3 p-md-4 overflow-hidden" id="dashboard">
            ...
            <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-3 mb-4">
                <div>
                    ...
                    <p class="text-muted small mb-0">Atualização dos dados da localização hospitalar.</p>
                </div>
            </div>
            ...
            <section class="mb-4">
                <div class="card p-4">
                    <form id="formLocalizacao" action="#" method="post" novalidate>
                        <input type="hidden" name="id_localizacao" value="<?php echo htmlspecialchars($idLocalizacaoEncrypted); ?>">
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control" id="codigoLocalizacao" name="codigoLocalizacao" placeholder="Ex: LOC-001"
                                    value="<?php echo valor_localizacao('codigoLocalizacao', $valores); ?>" required>
                            ...
                            <div class="col-12 d-flex flex-column flex-sm-row justify-content-end gap-2">
                                ...
                                <button type="submit" class="btn btn-primary" name="guardar_localizacao" value="1">
                                    <i class="fa-solid fa-floppy-disk me-1"></i> Atualizar localização
                                </button>
                            </div>
